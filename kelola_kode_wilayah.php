...
<body class="hold-transition sidebar-mini layout-fixed">
                                    ...
                                    <thead>
                                        <tr class="table-active">
                                            ...
                                            <th class="text-center">Kode Wilayah: <input class="cek form-control-sm" type="text" value="<?= $wilayah->kode_wilayah ?>" name="kode_wilayah[]" required>
                                                <button onclick="konfirmasiHapusPengadaan(event);" class="ml-2 btn text-cyan"><i class="fas fa-times"></i></button>
                                            </th>
                                            <input type="hidden" id="jumlah<?= $wilayah->kode_wilayah ?>" value="<?= count($rowlokasi) ?>" name="jumlahlokasiwilayah[]">
                                        <tr>
                                        ...
                                        ?>
                                            <tr>
                                                ...
                                                <td class="text-center"><input class=" cek form-control-sm text-center" type="text" value="<?= $lokasi->kode_lokasi ?>" name="kode_lokasi[]" required>
                                                    <button onclick="event.preventDefault(); $(this).parent().parent().remove(); $('#jumlah<?= $wilayah->kode_wilayah ?>').val(parseInt($('#jumlah<?= $wilayah->kode_wilayah ?>').val()) - 1); " class="ml-2 btn text-cyan"><i class="fas fa-times"></i></button>
                                                </td>
                                            </tr>
                                        <?php }
    ...
    <script>
        function tambahLokasi(id_target, id_jumlah) {
            var lokasibaru = `<tr>
                            ...
                            <button onclick="konfirmasiHapusPengadaan(event); event.preventDefault(); (()=>$(this).parent().parent().remove())(); $('#${id_jumlah}').val(parseInt($('#${id_jumlah}').val()) - 1);" class="ml-2 btn text-cyan"><i class="fas fa-times"></i></button></td>
                        </tr>`;
    ...
    <script>
        function konfirmasiHapusPengadaan(event) {
            jawab = true
            jawab = confirm('Yakin ingin menghapus? Data akan hilang permanen Jika Menekan Simpan!')

            if (jawab) {
                event.preventDefault();
                (() => $('#rowwilayah<?= $wilayah->kode_wilayah ?>').remove())();
                // return true
            } else {
                event.preventDefault()
                return false
        ...
        // function konfirmasiHapusLokasi(event, this) {
        //     jawab = true
        //     jawab = confirm('Yakin ingin menghapus? Data akan hilang permanen Jika Menekan Simpan!')
        ...
        //         $('#jumlah<?= $wilayah->kode_wilayah ?>').val(parseInt($('#jumlah<?= $wilayah->kode_wilayah ?>').val()) - 1);
        //     } else {
        //         event.preventDefault()
        //         return false
